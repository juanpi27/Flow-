...
public class Lexical {
	...
	String s = "Done";
        for(int i=0; i<tokens.size(); i++){
            ...
            if(isDelim(tokens.get(i))){
		tokensDef.add("Delimiter");
		tokensIden.add(tokens.get(i));
            } 
            else if(isStmt(tokens.get(i))){
		tokensDef.add("Keyword");
		tokensIden.add(tokens.get(i));
            } 
		...
		tokensIden.add(tokens.get(i));
                ...
                if(tokens.size()>i+2 && tokens.get(i+2).equals("'")){
                    if(isCharValue(tokens.get(i+1))){
                        tokensDef.add("Quote");
                        tokensIden.add("'");
                        tokensDef.add("CharValue");
                        tokensIden.add(""+(int)tokens.get(i+1).charAt(0));//returns ASCII value of a char
                        tokensDef.add("Quote");
                        tokensIden.add("'");
			i=i+2;
                    }
        ...
        while(st.hasMoreElements()){
            ...
            for(int i =0; i<c.length;i++){
                ...
                if(ch.toString().equals("//")){
                    for(int j=0;j<i;j++){
                        noComment=noComment+c[j];
                    }
                    return sReturn+noComment;
                }
            }
